"""Assemble the final original QA narrative from measured evidence."""
...
def main():
    browser=read(ROOT/'_preview/research-led-experience/browser/browser-audit.json')
    review=read(ROOT/'_preview/research-led-experience/review/review-qa.json')
    interactions=read(HERE/'interaction-qa.json')
    parity=read(HERE/'production-style-parity.json')
    validation=read(HERE/'validation-results.json')
    reconciliation=read(HERE/'compliance-reconciliation.json')
    fingerprint=read(HERE/'public-file-fingerprints.json')
    assert len(browser['results'])==568 and not any(browser['summary'].values())
    assert len(review['cases'])==40 and all(not c['contrast_failures'] and c['skip_link'] and c['focus_visible'] and c['text_200_percent_no_overflow'] and c['heading_200_percent_not_clipped'] for c in review['cases'])
    assert all(all(r['checks'].values()) for r in review['no_js'])
    assert interactions['case_count']==8 and interactions['failures']==0
    assert parity['overall_status']=='HEALTHY' and parity['public_pages']==71 and parity['referenced_live_assets']==168
    assert validation['deterministic'] and all(r['exit_code']==0 for r in validation['results'])
    assert reconciliation['protected_files_unchanged'] and reconciliation['human_review_text_unchanged']
    shutil.copyfile(ROOT/'_preview/research-led-experience/browser/browser-audit.json',HERE/'browser-audit.json')
    shutil.copyfile(ROOT/'_preview/research-led-experience/review/review-qa.json',HERE/'review-qa.json')
    contrast_total=sum(c['contrast_checked'] for c in review['cases'])
    manual=sum(c['contrast_manual_review'] for c in review['cases'])
    checked=datetime.now(timezone.utc).isoformat()
    text=f"""# Research-led experience — candidate QA

Review candidate only. **Not merged, not deployed.** Production baseline: `e0a668a10ddd6cafdf3f27ee4e6261f7e9caf27e`.

Prepared: {checked}. Branch: `agent/research-led-experience`.
...
Canonical public-file digest: `{fingerprint['sha256']}`.
...
The targeted scan examined {contrast_total} text candidates. It found zero failing contrast ratios among the cases it could resolve against solid backgrounds. {manual} cases involved gradients, images, opacity or another unsupported background and require human interpretation; representative page images were visually inspected. Do **not** present this as an automated WCAG pass.
...
"""
    (HERE/'QA_REPORT.md').write_text(text,encoding='utf-8',newline='\n')
    print(json.dumps({'qa':'PASS','public_digest':fingerprint['sha256'],'contrast_candidates':contrast_total,'manual_background_cases':manual}))
