...
def discovery_records(data: dict, library: dict, discovery: dict) -> list[dict]:
    ...
    for department in discovery["departments"]:
        records.append({"id":department["intentId"],"type":"department","title":department["title"],"summary":department["description"],"href":f'departments/{department["slug"]}.html',"terms":[department["title"],department["description"]]})
    ...
    for article in published_articles(library):
        ...
        records.append({"id":article["slug"],"type":"guide","title":article["title"],"summary":article["summary"],"href":f'library/{article["slug"]}.html',"category":category_name(library,article["category"]),"intents":related})
...
def compact_product_card(product: dict, *, prefix: str = "", index: int = 1) -> str:
    ...
    return f'''<article class="discovery-product" data-discovery-product data-id="{esc(product["id"],attribute=True)}" data-manufacturer="{esc(product["manufacturer"],attribute=True)}" data-intent="{esc(product["intent"],attribute=True)}" data-category="{esc(product["productKind"],attribute=True)}" data-name="{esc(product["name"].lower(),attribute=True)}" data-order="{index}"><img src="{prefix}{esc(image["src"],attribute=True)}" width="{int(image["width"])}" height="{int(image["height"])}" alt="{esc(image["alt"],attribute=True)}" loading="lazy" decoding="async"><div><p class="interface-label">{esc(product["manufacturer"])} / {esc(product["category"])}</p><h3>{esc(product["name"])}</h3>{price_markup(product,context="compact")}<div class="discovery-product__links"><a class="button button-primary" href="{esc(product["destination"],attribute=True)}" target="_blank" rel="sponsored noopener noreferrer" aria-label="Official product source for {esc(product["name"],attribute=True)} (opens in a new tab)">Official source ↗{external_note()}</a>{learn}</div></div></article>'''
...
def department_cards(data: dict, library: dict, discovery: dict, *, prefix: str = "") -> str:
    ...
    for item in discovery["departments"]:
        ...
        cards.append(f'''<a class="department-card" href="{prefix}departments/{item["slug"]}.html"><p class="interface-label">{esc(item["title"])}</p><strong>{product_count} products · {guide_count} guides</strong><span>{esc(item["description"])}</span><b>Explore department →</b></a>''')
    return "".join(cards)
...
def build_explore(data: dict, library: dict, discovery: dict) -> None:
    ...
    replacements={"{{DOCUMENT_HEAD}}":document_head_markup(metadata,prefix="",title="Explore Products and Education | The Mindful Matrix",description="Search verified products, testing tools, Library guides, and public education journeys in one place.",path=path,structured_data=[organization_schema(metadata),website_schema(metadata),breadcrumb_schema(metadata,[("Home",""),("Explore",path)])]),"{{SHARED_HEADER}}":shared_header_markup(data,prefix="",current="explore"),"{{SHARED_FOOTER}}":shared_footer_markup(data,prefix=""),"{{SEARCH}}":universal_search_markup(records),"{{DEPARTMENTS}}":department_cards(data,library,discovery),"{{CATALOG}}":explore_catalog_markup(products,data["catalog"])}
...
def build_departments(data: dict, library: dict, discovery: dict) -> None:
    ...
    for position,department in enumerate(discovery["departments"],start=1):
        ...
        replacements={"{{DOCUMENT_HEAD}}":document_head_markup(metadata,prefix="../",title=f'{department["title"]} | The Mindful Matrix',description=department["description"],path=path,structured_data=[organization_schema(metadata),website_schema(metadata),breadcrumb_schema(metadata,[("Home",""),("Explore","explore.html"),(department["title"],path)])]),"{{SHARED_HEADER}}":shared_header_markup(data,prefix="../",current="department"),"{{SHARED_FOOTER}}":shared_footer_markup(data,prefix="../"),"{{DEPARTMENT_INDEX}}":f'{position:02d}',"{{DEPARTMENT_ID}}":esc(department["intentId"],attribute=True),"{{DEPARTMENT_TITLE}}":esc(department["title"]),"{{DEPARTMENT_DESCRIPTION}}":esc(department["description"]),"{{PRODUCT_COUNT}}":str(len(matches)),"{{GUIDE_COUNT}}":str(len(guides)),"{{PRODUCTS}}":"".join(compact_product_card(p,prefix="../",index=i) for i,p in enumerate(matches,start=1)),"{{GUIDES}}":guides_markup,"{{JOURNEYS}}":journey_markup,"{{DISCLOSURE}}":esc(data["site"]["disclosure"])}
