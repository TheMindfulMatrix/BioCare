...
def validate_public_output(site: dict, library: dict, manifest: dict, preview_path: Path | None) -> None:
    ...
    check(home.count('data-image-role="official-product-cutout"') == 2, "Homepage must render only hero and active Product Universe cutouts initially; testing education links to its complete journey")
    check(home.count("data-price-record") == 2, "Homepage must retain hero pricing and active Product Universe pricing without a duplicate testing promotion")
    ...
    universe_markup = home[universe_start:home.find('<section id="know-your-number"')]
    ...
    check(home.count("data-library-article") == min(3, len([article for article in library["articles"] if article.get("status") == "published"])), "Homepage previews up to three published guides; the Library retains the full collection")
    ...
    check('data-pathfinder' in home and home.count('data-path-panel=') == 3, "Homepage must expose all three learning paths in the static HTML")
    check(home.find('<section id="shelf"') < home.find('<section id="matrix"'), "Product exploration must remain directly accessible before the longer editorial method")
    ...
    shop = (ROOT / "shop.html").read_text(encoding="utf-8"); shop_catalog_start = shop.find('<div class="catalog-grid"')
    shop_payload_match = re.search(r'<script type="application/json" data-shop-catalog>(.*?)</script>', shop, flags=re.S)
    check(bool(shop_payload_match), "Shop requires one build-generated V9 catalog payload")
    try:
        shop_payload = json.loads(shop_payload_match.group(1)) if shop_payload_match else {}
    except json.JSONDecodeError as error:
        check(False, f"Shop V9 catalog payload is invalid JSON: {error}")
        shop_payload = {}
    shop_products = shop_payload.get("products", []) if isinstance(shop_payload, dict) else []
    shop_by_id = {record.get("id"): record for record in shop_products if isinstance(record, dict) and record.get("id")}
    initial_products = active_products[:12]
    check(shop.count(affiliate_disclosure) == 1, "Shop must contain the exact affiliate disclosure once")
    check(shop_catalog_start < shop.find(affiliate_disclosure), "Shop full affiliate disclosure must remain available after product browsing")
    check('Official manufacturer links may earn The Mindful Matrix a commission.' in shop[:shop_catalog_start], "Shop requires a readable compact commission disclosure before the catalog")
    check(shop.count(biolimitless_disclosure) == 1, "Shop must retain the exact BioLimitless relationship disclosure globally")
    check(shop.count(pricing_disclosure) == 1 and shop_catalog_start < shop.find(pricing_disclosure), "Shop pricing disclosure must remain complete after the catalog")
    check(shop.count(site["site"]["fdaDisclaimer"]) == 2 and shop_catalog_start < shop.find(site["site"]["fdaDisclaimer"]), "Shop must retain the complete FDA disclaimer after the catalog and in the footer")
    ...
    check(all(token in shop for token in ('name="filter-manufacturer" value="all"','name="filter-manufacturer" value="Zinzino"','name="filter-manufacturer" value="BioLimitless"')), "Shop requires derived All, Zinzino and BioLimitless manufacturer filters")
    check(all(token in shop for token in ('data-shop-filter-dialog','data-product-inspector','data-shop-load-more','data-shop-chips')), "Shop requires the V9 filter sheet, reusable inspector, progressive rendering and active chips")
    check('price.pricing_model === "starter_subscription"' in enhancements_text and 'price.pricing_model === "one_time_autoship"' in enhancements_text, "V9 client rendering must preserve unlike canonical price models")
    check('rel="sponsored noopener noreferrer" aria-describedby="inspector-source-disclosure"' in enhancements_text, "V9 inspector commercial links must remain sponsored and disclosure-bound")
    ...
    for index, product in enumerate(active_products):
        record = shop_by_id.get(product["id"], {})
        check(bool(record), f"{product['id']}: missing from V9 canonical client payload")
        check(record.get("index") == index + 1 and record.get("manufacturer") == product["manufacturer"] and record.get("productKind") == product["productKind"], f"{product['id']}: V9 payload canonical metadata mismatch")
        check(record.get("description") == product["description"] and record.get("price", {}).get("pricing_model") == product["price"]["pricing_model"], f"{product['id']}: V9 payload content or pricing mismatch")
        if index < 12:
            check(shop.count(f'data-product-id="{product["id"]}"') == 1, f"{product['id']}: initial compact card must be unique")
            check(shop.count(f'id="product-{product["id"]}"') == 1, f"{product['id']}: initial compact anchor must be unique")
        else:
            check(f'data-product-id="{product["id"]}"' not in shop, f"{product['id']}: below-fold card must be progressively rendered rather than hidden in the DOM")
        check(product["destination"] in shop, f"{product['id']}: destination missing from Shop")
        source=attributed_source_url(product); check(record.get("price", {}).get("affiliate_price_source") == source, f"{product['id']}: attributed official source mismatch in V9 payload")
        check(record.get("cutout", {}).get("src") == product["cutout"]["src"], f"{product['id']}: official foreground image mismatch in V9 payload")
