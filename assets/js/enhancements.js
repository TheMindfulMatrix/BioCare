(function () {
  ...
  function initMatrixSearch(root) {
    ...
    modes.forEach(function(button){button.addEventListener("click",function(){var nextMode=button.dataset.searchMode;if(nextMode===mode)return;mode=nextMode;render(false);save(false);});});input.addEventListener("input",function(){render(false);});clear.addEventListener("click",function(){input.value="";render(false);save(false);input.focus();});form.addEventListener("submit",function(event){if(location.pathname.endsWith("explore.html")){event.preventDefault();render(false);save(false);}});root.addEventListener("keydown",function(event){if(event.key==="Escape"&&!results.hidden){input.value="";render(false);save(false);input.focus();}});
  ...
  if (dock) {
    ...
    var section = path === "index.html" ? "home" : path === "explore.html" ? "explore" : path === "shop.html" || location.pathname.indexOf("/products/") !== -1 ? "shop" : path === "library.html" || location.pathname.indexOf("/library/") !== -1 ? "library" : path === "start.html" ? "start" : "";
    var currentDock = dock.querySelector('[data-dock-page="' + section + '"]');
    if (currentDock) currentDock.setAttribute("aria-current", "page");
  }

  var libraryControls = document.querySelector("[data-library-controls]");
  if (libraryControls) {
